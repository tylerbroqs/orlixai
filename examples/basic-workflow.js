 ...
 *
 * Run:  node examples/basic-workflow.js
 */
...
const goal = orlix.memory.addGoal({
  name:     'Ship MVP',
  deadline: new Date(Date.now() - 2 * 86_400_000).toISOString().slice(0, 10), // 2 days ago (overdue)
  progress: 0.6,
});
...
const email = new EmailIntegration({
  inbox: [
    { from: 'alice@example.com', subject: 'Re: launch plan', preview: 'LGTM, let\'s ship!', read: false, receivedAt: new Date(Date.now() - 2 * 3_600_000).toISOString() },
    { from: 'bob@example.com',   subject: 'Blocker on auth',  preview: 'The OAuth is broken.', read: false, receivedAt: new Date(Date.now() - 30 * 3_600_000).toISOString() },
  ],
...
orlix.loop.on('observe',  signals   => logger.ok(`Observed ${signals.length} signal(s) from integrations`));
orlix.loop.on('decide',   decisions => logger.ok(`${decisions.length} decision(s) from policy engine`));
orlix.loop.on('act',      receipt   => { console.log(''); logger.receipt(receipt); });
orlix.loop.on('verify',   receipt   => logger.ok(`Verified receipt: ${receipt.id}`));
orlix.loop.on('learn',    upd       => logger.ok(`Policy update: ${upd.policy?.rule} — ${upd.reason}`));
