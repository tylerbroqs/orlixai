 ...
 *
 * Run:  node examples/custom-policy.js
 */
...
orlix.policyEngine.register('weekly_review_reminder', (worldModel) => {
  const today = new Date().getDay();
  if (today !== 5) return null; // only Fridays
...
orlix.loop.on('act',    r => logger.receipt(r));
orlix.loop.on('decide', d => d.forEach(x => logger.decision(x)));

const result = await orlix.tick();
logger.section('done');
logger.ok(`Signals: ${result.signals.length}  Decisions: ${result.decisions.length}  Actions: ${result.receipts.length}`);
