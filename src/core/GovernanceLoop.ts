...
import type { Tier, Decision, Receipt, PolicyUpdate, LoopResult, ActionHandler } from '../types/index.js';
...
export class GovernanceLoop extends EventEmitter {
  ...
  constructor(opts: GovernanceLoopOptions) {
    ...
    this.actionHandlers = opts.actionHandlers ?? new Map();
  }
  ...
  pendingApprovals(): Decision[] { return [...this._pending]; }
    ...
    for (const [, handler] of this.actionHandlers) {
      if (typeof handler?.observe === 'function') {
        try {
          ...
          s.forEach((sig) => { this.worldModel.ingestSignal(sig); signals.push(this.worldModel.signals[0]!); });
        } catch (err) {
  ...
  private async _learn(receipts: Receipt[]): Promise<PolicyUpdate[]> {
    ...
    for (const receipt of receipts) {
      ...
      for (const policy of this.worldModel.activePolicies()) {
        ...
        if (!fb) continue;
        const updated = this.memory.updatePolicy(policy.id, fb);
  ...
  private async _execute(decision: Decision, approval: Receipt['approval']): Promise<Receipt> {
    ...
    if (handler?.execute) {
      try { await handler.execute(decision, receipt); }
      catch (err) { this.auditLog.fail(receipt.id, (err as Error).message); throw err; }
    }
    return receipt;
  }

  private _feedback(receipt: Receipt): ({ learnReason: string } & Record<string, unknown>) | null {
    if (receipt.outcome?.includes('confirmed'))
      return { learnReason: 'user confirmed — policy reinforced' };
    if (receipt.outcome?.includes('rejected') || receipt.outcome?.includes('override'))
      return { status: 'paused', learnReason: 'user rejected — policy paused for review' };
    return null;
  }
}
