...
export class CalendarIntegration extends BaseIntegration {
  ...
  async observe(): Promise<Omit<Signal, 'id'>[]> {
    ...
    for (const event of events) {
      ...
      if (minLeft > 0 && minLeft <= this._window) {
        signals.push(this.signal('calendar_event', {
          title:     event.title,
      ...
      if (event.blocksWork && minLeft > 0 && minLeft <= 120) {
        signals.push(this.signal('calendar_block', {
          title:    event.title,
  ...
  async verify(receipt: Receipt): Promise<string> {
    return receipt.action === 'send_calendar_reminder' ? 'reminder delivered' : 'completed';
  }

  /** Override to connect to a real calendar API. */
  protected async _fetch(): Promise<CalendarEvent[]> {
    return this._events;
  }
}
